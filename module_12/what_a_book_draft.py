...
# if statements for basic exception handling and directing the application to the correct code sections. Could be improved with while loop. Maybe later.
...
# basic exception handling for incorrect input. Again a while loop would help with EH, but that will have to come later. 
    if user_num < 1 or user_num > 3:
        print('Whoopsie doodles, something went wrong. Please try again...')
        sleep(2)
        account_info()
    else:
        user_first = 'select first_name from user where user_id = {}'.format(user_num)
...
def account_menu():
    ...
    user_choice = input("Please enter the number of the option you would like to view: \n")
